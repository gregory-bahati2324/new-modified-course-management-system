import { API_CONFIG_SCHEDULING} from "../config/api.config";
...
export const apiSchedulingClient: AxiosInstance = axios.create({
  baseURL: API_CONFIG_SCHEDULING.baseURL, // port 8006
  timeout: API_CONFIG_SCHEDULING.timeout,
  headers: { 'Content-Type': 'application/json' },
});

// Attach JWT token automatically to each request
apiSchedulingClient.interceptors.request.use(config => {
...
apiSchedulingClient.interceptors.response.use(
  response => response,
  async error => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        if (newToken) {
          ...
          return apiSchedulingClient(originalRequest);
        }
      } catch (refreshError) {
