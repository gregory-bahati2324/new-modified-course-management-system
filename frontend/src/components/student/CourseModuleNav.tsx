...
export function CourseModuleNav({
    ...
    const getLessonType = (lesson: NavLesson) => {
        if (lesson.contentBlocks?.some(b => b.type === 'video')) return 'video';
        if (lesson.quizQuestions?.length) return 'quiz';
        return 'reading';
    };
    ...
    const getModuleProgress = (module: NavModule) => {
                    ...
                    {modules.map((module) => {
                        ...
                        return (
                            <Collapsible
                                key={module.id}
                                open={isExpanded}
                                onOpenChange={() => !module.locked && toggleModule(module.order)}
                            >
                                <CollapsibleTrigger
                                    className={cn(
                                ...
                                <CollapsibleContent className="pl-4 pr-2 pb-2">
                                    <div className="space-y-0.5 pt-1 border-l-2 border-border ml-2">
                                        {module.lessons.map((lesson) => {
                                            const completed = lesson.is_completed ?? false;
